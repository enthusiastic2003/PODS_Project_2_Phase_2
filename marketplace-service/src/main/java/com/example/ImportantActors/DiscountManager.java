...
public class DiscountManager extends AbstractBehavior<DiscountManager.Command> {
    ...
    public static class GetDiscountResponse implements Response {
        boolean discount_availed;
        public GetDiscountResponse(boolean discount_availed) {
            this.discount_availed = discount_availed;
        }
    }

    // Command to check a user's discount status
    public static class GetDiscountStatus implements Command {
        Integer user_id;
        ActorRef<GetDiscountResponse> replyTo;
        boolean presumed_discount_status;  // Default status if user not found

        public GetDiscountStatus(Integer user_id, boolean presumed_discount_status,
                                 ActorRef<GetDiscountResponse> replyTo) {
    ...
    public static class ApplyDiscount implements Command {
        Integer user_id;
        ActorRef<GetDiscountResponse> replyTo;

        public ApplyDiscount(Integer user_id, ActorRef<GetDiscountResponse> replyTo) {
            this.user_id = user_id;
            this.replyTo = replyTo;
        }
    }
    ...
    public static class ReleaseDiscount implements Command {
        Integer user_id;

        public ReleaseDiscount(Integer user_id) {
            this.user_id = user_id;
        }
    }

    // Command to revert a discount lock
    public static class RevertDiscountLock implements Command {
        Integer user_id;

        public RevertDiscountLock(Integer user_id) {
            this.user_id = user_id;
        }
    }
    ...
    public Behavior<Command> onRevertDiscountLock(RevertDiscountLock revertDiscountLock) {
        // Only revert if currently locked
    ...
    public Behavior<Command> onReleaseDiscount(ReleaseDiscount command) {
        ...
        // Async HTTP call to update external service
    ...
    public Behavior<Command> onApplyDiscount(ApplyDiscount msg) {
        ...
        // Only apply if discount not already active
