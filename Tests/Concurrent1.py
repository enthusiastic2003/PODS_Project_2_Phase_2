...
"""
*Discount only once, when concurrent orders form same user*
This test case puts multiple concurrent orders from the same user.
And checks if the discount is applied only once.
"""


Number_of_discounts_availed = 0
successful_orders = 0
...
def main():
    try:
        ...
        if not check_response_status_code(resp, 200):
            return False
        correct_final_balance = initial_balance - (original_price * (successful_orders-1)) - discount_price
        print("correct_final_balance",correct_final_balance)
        if not test_get_wallet(user_id, resp, correct_final_balance):
           
            print("\033[91mDiscount used multiple times by single user\033[0m")
            return False


        delete_user(user_id)    
     
        print("\033[92mDiscount used only once by single user\033[0m")                 
